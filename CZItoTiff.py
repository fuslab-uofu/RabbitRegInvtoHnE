...
save_dirpath=os.path.join(BaseSavePath, rab_ID, "HnE", "Block0"+str(block_no))
...
#Path to CSV-
csv_dirpath=os.path.join(BaseCephPath, rab_ID, rab_ID + "_BlockFaceImages","block0" +str(block_no),"csv_files")
...
czi_dirpath=os.path.join(BaseCephPath, rab_ID, rab_ID + "_HnE_5x", "block0"+str(block_no))
for file in os.listdir(czi_dirpath):
    #Can edit this string for funky naming stuff that may go on or files saved in the wrong place-
    if "_" in file:
        print(file)
        ...
        #Can comment this out if you dont want to look at them all->
        plt.imshow(czi_img)
        plt.show()

        #Save Image as Tiff-
        new_file_path = os.path.join(save_dirpath, "HnE_IMG_"+ image_tag + ".tif")
        print("Tiff saved to ", new_file_path)
